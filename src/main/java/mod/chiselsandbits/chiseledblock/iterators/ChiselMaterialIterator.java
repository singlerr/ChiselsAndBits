...
public class ChiselMaterialIterator extends BaseChiselIterator implements ChiselIterator {
  ...
  public ChiselMaterialIterator(
    ...
    if (place) {
      ...
      z -= tz;
      placeoffsetX =
          side.getAxis() == Axis.X ? side.getAxisDirection() == AxisDirection.POSITIVE ? 1 : -1 : 0;
      placeoffsetY =
          side.getAxis() == Axis.Y ? side.getAxisDirection() == AxisDirection.POSITIVE ? 1 : -1 : 0;
      placeoffsetZ =
          side.getAxis() == Axis.Z ? side.getAxisDirection() == AxisDirection.POSITIVE ? 1 : -1 : 0;
    }
    ...
    while (bi.hasNext()) {
      if (source.getSafe(bi.x - tx, bi.y - ty, bi.z - tz) == target) {
        ...
        if (xx >= 0 && xx < VoxelBlob.dim && yy >= 0 && yy < VoxelBlob.dim && zz >= 0 &&
            zz < VoxelBlob.dim) {
