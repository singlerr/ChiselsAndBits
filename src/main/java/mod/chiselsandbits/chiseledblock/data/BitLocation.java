...
public class BitLocation implements IBitLocation {
  ...
  public BitLocation(final BlockHitResult mop, final BitOperation type) {
    ...
    final Vec3i inBlockPos =
        new Vec3i((int) inBlockPosAccurate.x(), (int) inBlockPosAccurate.y(),
            (int) inBlockPosAccurate.z());
    final Vec3i normalizedInBlockPos = new Vec3i(
        snapToValid(inBlockPos.getX()), snapToValid(inBlockPos.getY()),
        snapToValid(inBlockPos.getZ()));
    final Vec3i normalizedInBlockPosWithOffset =
        type.usePlacementOffset() ? normalizedInBlockPos.relative(mop.getDirection(), 1) :
