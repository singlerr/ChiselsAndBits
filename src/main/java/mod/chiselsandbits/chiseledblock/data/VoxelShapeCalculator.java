  ...
  private static VoxelShape calculateFromBB(final Collection<AABB> bbList) {
    return bbList.stream()
        .reduce(
            ...
            (voxelShape, voxelShape2) -> Shapes.joinUnoptimized(voxelShape, voxelShape2,
                BooleanOp.OR))
        .optimize();
  }
      ...
      for (int z = 0; z < 16; z++) {
        ...
        for (int y = 0; y < 16; y++) {
          ...
          if (blob.get(x, y, z) != 0) {
            ...
            collisionShape = Shapes.or(
                collisionShape,
                Shapes.box(dx, start / 16.0d, dz, dx + bitDimension, y / 16.0d, dz + bitDimension));
            start = -1;
            if (x < x1) {
              x1 = x;
            }
        ...
        if (start != -1) {
          ...
          collisionShape = Shapes.or(
              collisionShape,
              Shapes.box(dx, start / 16.0d, dz, dx + bitDimension, 1.0d, dz + bitDimension));
          y2 = 15;
          if (x < x1) {
            x1 = x;
          }
