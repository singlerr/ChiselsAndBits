...
public class ChiselModeManager {
  ...
  public static void changeChiselMode(
      final ChiselToolType tool, final IToolMode originalMode,
    ...
    if (ChiselsAndBits.getConfig().getClient().perChiselMode.get() && tool.hasPerToolSettings()
      ...
      final PacketSetChiselMode packet =
          new PacketSetChiselMode(newClientChiselMode, tool, chatNotification);
  ...
  public static void scrollOption(
      final ChiselToolType tool, final IToolMode originalMode, IToolMode currentMode,
      final int dwheel) {
