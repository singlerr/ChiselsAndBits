...
public class VoxelRegionSrc implements IVoxelSrc {
    ...
    for (int x = min.getX(); x <= max.getX(); ++x) {
      for (int y = min.getY(); y <= max.getY(); ++y) {
        for (int z = min.getZ(); z <= max.getZ(); ++z) {
          final int idx =
              x - min.getX() + (y - min.getY()) * wrapX + (z - min.getZ()) * wrapX * wrapY;
  ...
  public VoxelRegionSrc(final Level theWorld, final BlockPos blockPos, final int range) {
    this(theWorld, blockPos.offset(-range, -range, -range), blockPos.offset(range, range, range),
        blockPos);
  }
  ...
  public int getSafe(int x, int y, int z) {
    ...
    if (blkPosX < 0 || blkPosY < 0 || blkPosZ < 0 || blkPosX >= wrapX || blkPosY >= wrapY ||
        blkPosZ >= wrapZ) {
      return 0;
    }
