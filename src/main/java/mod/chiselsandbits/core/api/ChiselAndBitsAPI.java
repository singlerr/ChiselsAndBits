...
public class ChiselAndBitsAPI implements IChiselAndBitsAPI {
  ...
  public void giveBitToPlayer(final Player player, final ItemStack stack, Vec3 spawnPos) {
    ...
    final ItemEntity ei =
        new ItemEntity(player.getCommandSenderWorld(), spawnPos.x, spawnPos.y, spawnPos.z, stack);
  ...
  public IBitBag getBitbag(final ItemStack stack) {
    if (!ModUtil.isEmpty(stack)) {
      final Object o =
          stack.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, Direction.UP);
  ...
  public void renderGhostModel(
      ...
      final int combinedLight,
      final int combinedOverlay) {
    RenderHelper.renderGhostModel(stack, model, world, pos, isUnplaceable, combinedLight,
        combinedOverlay);
  }
}
