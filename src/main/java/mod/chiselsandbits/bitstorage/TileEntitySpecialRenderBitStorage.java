...
public class TileEntitySpecialRenderBitStorage
    implements BlockEntityRenderer<TileEntityBitStorage> {
  ...
  public void render(
    ...
    VoxelBlob innerModelBlob =
        STORAGE_CONTENTS_BLOB_CACHE.get(new CacheKey(ModUtil.getStateId(state), bits));
    if (innerModelBlob == null) {
      innerModelBlob = new VoxelBlob();
      innerModelBlob.fillAmountFromBottom(ModUtil.getStateId(state), bits);
      STORAGE_CONTENTS_BLOB_CACHE.put(new CacheKey(ModUtil.getStateId(state), bits),
          innerModelBlob);
    }
    ...
    RenderType.chunkBufferLayers().forEach(renderType -> {
      ...
      if (!innerModel.isEmpty()) {
        final float r =
            te.getMyFluid() == null ? 1f :
                ((FluidUtil.getColor(te.getMyFluid()) >> 16) & 0xff) / 255F;
        final float g =
            te.getMyFluid() == null ? 1f :
                ((FluidUtil.getColor(te.getMyFluid()) >> 8) & 0xff) / 255f;
        final float b =
            te.getMyFluid() == null ? 1f : ((FluidUtil.getColor(te.getMyFluid())) & 0xff) / 255f;
