...
public class DataAwareChiseledBlockBakedModel extends BaseSmartModel {
    ...
    public Set<ChiselRenderType> getRenderTypes(
            ...
            @NotNull BlockState state,
            @NotNull IModelData modelData) {
        if (!(world.getBlockEntity(pos) instanceof TileEntityBlockChiseled te)) return Set.of(ChiselRenderType.SOLID);

        if (te.getBlob() == null) return Set.of(ChiselRenderType.SOLID);
