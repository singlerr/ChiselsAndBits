...
public class ModelDataManager {
  private static final Map<ChunkPos, Set<BlockPos>> needModelDataRefresh =
      new ConcurrentHashMap<>();
  private static final Map<ChunkPos, Map<BlockPos, IModelData>> modelDataCache =
      new ConcurrentHashMap<>();
  ...
  public static void requestModelDataRefresh(BlockEntity te) {
    ...
    needModelDataRefresh
        .computeIfAbsent(new ChunkPos(te.getBlockPos()),
  ...
  private static void refreshModelData(BlockAndTintGetter level, ChunkPos chunk) {
    ...
    if (needUpdate != null) {
      Map<BlockPos, IModelData> data =
          modelDataCache.computeIfAbsent(chunk, $ -> new ConcurrentHashMap<>());
