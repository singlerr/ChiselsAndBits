...
public class ChiselsAndBitsMenu extends Screen {
  ...
  public void render(
    ...
    for (final MenuRegion mnuRgn : modes) {
      ...
      final double x =
          (mnuRgn.x1 + mnuRgn.x2) * 0.5 * (ring_outer_edge * 0.6 + 0.4 * ring_inner_edge);
      final double y =
          (mnuRgn.y1 + mnuRgn.y2) * 0.5 * (ring_outer_edge * 0.6 + 0.4 * ring_inner_edge);
    ...
    for (final MenuRegion mnuRgn : modes) {
      if (mnuRgn.highlighted) {
        ...
        graphics.drawString(font, text, (int) middle_x + fixed_x, (int) middle_y + fixed_y,
            0xffffffff);
      }
    }
