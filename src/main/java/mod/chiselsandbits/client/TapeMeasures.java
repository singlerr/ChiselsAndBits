...
public class TapeMeasures {
  ...
  private static double getLineDistance(final Vec3 v, final Vec3 w, final Player player,
  ...
  private void renderMeasure(
      final Measure m, final double distance, final PoseStack matrixStack,
      final float partialTicks) {
    final Player player = ClientSide.instance.getPlayer();
    ...
    if (m.mode == TapeMeasureModes.DISTANCE) {
      ...
      RenderHelper.drawLineWithColor(
          matrixStack, a, b, BlockPos.ZERO, player, partialTicks, false, red, green, blue, alpha,
          (int)
              (alpha / 3.4));
  ...
  private void billBoard(final PoseStack matrixStack, final Player player,
                         final float partialTicks) {
