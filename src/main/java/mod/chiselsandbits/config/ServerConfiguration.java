...
public class ServerConfiguration extends AbstractConfiguration {
  ...
  protected ServerConfiguration(final ForgeConfigSpec.Builder builder) {
    ...
    logEligibilityErrors =
        defineBoolean(builder, "server.troubleshooting.logging.eligibility-errors", true);
    ...
    enableToolHarvestLevels =
        defineBoolean(builder, "server.balancing.tools.harvest-levels.enabled", true);
    enableBitLightSource =
        defineBoolean(builder, "server.balancing.bits.act-as-light-source", true);
    bitLightPercentage = defineDouble(builder, "server.balancing.bits.light-percentage", 6.25);
    compatabilityMode =
        defineBoolean(builder, "server.balancing.compatibility-mode.enabled", false);
    ...
    fullBlockCrafting =
        defineBoolean(builder, "server.balancing.full-block-crafting.enabled", true);
    ...
    revertibleBlocks = defineList(
        builder, "server.balancing.revertible.blocks", Lists.newArrayList("*"),
  ...
  public boolean canRevertToBlock(BlockState newState) {
    ...
    return blockNames.contains("*")
        || blockNames.contains(
        Objects.requireNonNull(BuiltInRegistries.BLOCK.getKey(newState.getBlock()))
            .toString());
  }
}
