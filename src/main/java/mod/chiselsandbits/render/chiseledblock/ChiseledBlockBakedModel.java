...
public class ChiseledBlockBakedModel extends BaseBakedBlockModel {
  ...
  public ChiseledBlockBakedModel(
      final int blockReference, final ChiselRenderType layer, final VoxelBlob data,
      final VertexFormat format) {
    myLayer = layer;
    final BlockState state = ModUtil.getStateById(blockReference);
  ...
  private static void offsetVec(
      final int[] result, final int toX, final int toY, final int toZ, final Direction f,
      final int d) {
  ...
  public static ChiseledBlockBakedModel createFromTexture(TextureAtlasSprite findTexture,
  ...
  private void generateFaces(final ChiseledModelBuilder builder, final VoxelBlob blob,
  ...
  private void processXFaces(
      final VoxelBlob blob, final VisibleFace visFace,
  ...
  private void getVertexPos(
      final float[] pos, final Direction side, final int vertNum, final int[] to,
      final int[] from) {
  ...
  private void getFaceUvs(
      final float[] uvs, final Direction face, final int[] from, final int[] to,
      final float[] quadsUV) {
    float to_u = 0;
    float to_v = 0;
    float from_u = 0;
